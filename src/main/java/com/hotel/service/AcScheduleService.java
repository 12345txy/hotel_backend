...
import com.hotel.mapper.AirConditionerMapper;
...
import com.hotel.entity.QueueStatus;
...
public class AcScheduleService {
    ...
    @Value("${hotel.ac.total-count:3}")
    private int totalAcCount;
    
    @Value("${hotel.ac.time-slice:120}")
    private long timeSliceSeconds;
    
    private final ReentrantLock scheduleLock = new ReentrantLock();
    
    // 服务队列：优先级最低的在队头（最容易被换出）
    private final PriorityQueue<AcRequest> serviceQueue = new PriorityQueue<>(
        Comparator.comparing(AcRequest::getFanSpeedPriority)          // 风速低的优先（容易被换出）
            .thenComparing(AcRequest::getServiceTime, Comparator.reverseOrder()) // 服务时间长的优先（容易被换出）
            .thenComparing(AcRequest::getRoomNumber, Comparator.reverseOrder())   // 房间号大的优先（容易被换出）
    );
    
    // 等待队列：优先级最高的在队头（最优先换入）
    private final PriorityQueue<AcRequest> waitingQueue = new PriorityQueue<>(
        Comparator.comparing(AcRequest::getFanSpeedPriority).reversed() // 风速高的优先（优先换入）
            .thenComparing(AcRequest::getWaitTime, Comparator.reverseOrder()) // 等待时间长的优先（优先换入）
            .thenComparing(AcRequest::getRoomNumber)                     // 房间号小的优先（优先换入）
    );
    
    // 房间请求映射
    private final ConcurrentHashMap<Integer, AcRequest> roomRequestMap = new ConcurrentHashMap<>();
    ...
    public static class AcRequest {
        ...
        private Long serviceTime; // 服务时长(秒)
        private Long waitTime;    // 等待时长(秒)
        private Long acId;
        private boolean inService;
        
        public int getFanSpeedPriority() {
            return FanSpeed.fromCode(fanSpeed).getPriority();
        }
        
        public void updateServiceTime() {
            if (serviceStartTime != null) {
                this.serviceTime = java.time.Duration.between(serviceStartTime, LocalDateTime.now()).getSeconds();
            } else {
                this.serviceTime = 0L;
            }
        }
        ...
        public void updateWaitTime() {
            if (requestTime != null && !inService) {
                this.waitTime = java.time.Duration.between(requestTime, LocalDateTime.now()).getSeconds();
            } else {
                this.waitTime = 0L;
            }
        }
     ...
     * 添加空调请求
     */
    public String addAcRequest(Room room) {
        scheduleLock.lock();
        try {
            log.info("房间{}请求空调服务", room.getId());
            
            AcRequest request = new AcRequest();
            request.setRoomNumber(room.getId().intValue());
            request.setRoomId(room.getId());
            request.setFanSpeed(room.getFanSpeed());
            request.setMode(room.getAcMode());
            ...
            request.setInService(false);
                    ...
                    roomRequestMap.put(room.getId().intValue(), request);
    ...
    public String adjustAcRequest(Integer roomNumber, String fanSpeed, String mode, Double targetTemp) {
        ...
        try {
            ...
            AcRequest request = roomRequestMap.get(roomNumber);
            if (request == null) {
                return "房间未开启空调";
            }
    ...
    public String removeAcRequest(Integer roomNumber, String reason) {
        ...
        try {
            ...
            AcRequest request = roomRequestMap.remove(roomNumber);
            if (request == null) {
                return "房间未开启空调";
            }
     ...
     */
    private void addNewRequest(AcRequest request) {
        if (serviceQueue.size() < totalAcCount) {
            ...
            AcRequest lowestPriorityInService = serviceQueue.peek();
    ...
    private void rebuildQueuesWithUpdatedPriorities() {
        // 保存当前队列中的请求
        List<AcRequest> currentServiceRequests = new ArrayList<>(serviceQueue);
        List<AcRequest> currentWaitingRequests = new ArrayList<>(waitingQueue);
     ...
     */
    private void performPrioritySchedule(AcRequest highPriorityRequest, AcRequest lowPriorityRequest) {
        log.info("优先级调度：房间{}(风速{})抢占房间{}(风速{})", 
            highPriorityRequest.getRoomNumber(), highPriorityRequest.getFanSpeed(),
            lowPriorityRequest.getRoomNumber(), lowPriorityRequest.getFanSpeed());
     ...
     */
    @Transactional
    private void assignAcToRequest(AcRequest request) {
     ...
     */
    @Transactional
    private void addToWaitingQueue(AcRequest request) {
        request.setInService(false);
        request.setWaitTime(0L);
    ...
    private void syncQueueStatusToDatabase() {
        try {
            ...
            for (AcRequest request : serviceQueue) {
                Room room = roomMapper.selectById(request.getRoomId());
                if (room != null) {
            ...
            for (AcRequest request : waitingQueue) {
                Room room = roomMapper.selectById(request.getRoomId());
                if (room != null) {
    ...
    public void checkTimeSliceRotation() {
        ...
        // 队头就是最优先换入的等待请求
        AcRequest highestWaitingRequest = waitingQueue.peek();
        
        if (highestWaitingRequest != null && highestWaitingRequest.getWaitTime() >= timeSliceSeconds) {
            // 队头就是最容易被换出的服务请求
            AcRequest lowestServiceRequest = serviceQueue.peek();
     ...
     * 获取当前服务队列
     */
    public PriorityQueue<AcRequest> getServiceQueue() {
     ...
     * 获取当前等待队列
     */
    public PriorityQueue<AcRequest> getWaitingQueue() {
     ...
     * 获取房间的请求状态
     */
    public AcRequest getRoomRequest(Integer roomNumber) {
     ...
     */
    private Map<String, Object> createRoomInfoFromRequest(AcRequest request, Room room) {
     ...
     * 格式化队列项目信息
     */
    private Map<String, Object> formatQueueItem(AcRequest request) {
        Map<String, Object> item = new HashMap<>();
        item.put("roomId", request.getRoomId());
        item.put("roomNumber", request.getRoomNumber());
        item.put("fanSpeed", request.getFanSpeed());
